...
- `open_fee = OA_SPOT_FEE_RATE×spot + OA_OPTION_FEE_RATE×(call + put)`;`open_net = open_gross − open_fee`
...
- **平仓条件**:`往返净利率 ≥ OA_CLOSE_RATE_DISCOUNT × open_rate`(默认 0.6,即锁定率打 6 折即平)

**到期结算(现金交割)** — 到期前未达标平仓则持有到交割:

- `结算净利 = open_gross − open_fee − OA_SPOT_FEE_RATE×P − OA_EXERCISE_FEE_RATE×P`
  (`P` ≈ 现货中间价;现货腿按结算价平回 + ITM 期权腿扣行权费,近似,忽略 10% 封顶)

> reversal 需融币做空现货、conversion 需 USDT 买现货,借贷与资金成本、滑点均未计入;
> 现货腿用 `OA_SPOT_FEE_RATE`、期权腿用 `OA_OPTION_FEE_RATE` 近似(未建模期权「指数价×0.03% 且封顶权利金 10%」的精确规则)。
...
| `OA_CLOSE_RATE_DISCOUNT` | `0.6` | 平仓折扣：往返净利率 ≥ 系数 × 开仓锁定率 时平仓 |
| `OA_MAX_OPEN_MARK_LOSS` | `0.001` | 开仓可平性门槛：开仓瞬间「立即反向平仓」的往返亏损率超过该值（点差过宽、平不掉）则放弃开仓 |
